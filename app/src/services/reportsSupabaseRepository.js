...
function mapSupabaseReportRow(row) {
  return {
    ...
    // Penilaian AI dibawa berdampingan dengan baseline, tidak menggantikannya.
    aiRiskScore: Number.isFinite(Number(row.ai_risk_score)) ? Number(row.ai_risk_score) : null,
    aiRiskReason: row.ai_risk_reason || '',
    aiRecommendations: Array.isArray(row.ai_recommendations) ? row.ai_recommendations : [],
    aiModel: row.ai_model || '',
    aiAssessedAt: row.ai_assessed_at || '',
    upstreamKecamatan: row.upstream_kecamatan || '',
    upstreamRainfallMm: Number.isFinite(Number(row.upstream_rainfall_mm)) ? Number(row.upstream_rainfall_mm) : null,
